const relevantParams = ['arrival', 'departure', 'adults1', 'children1', 'locale', 'currency', 'nbNightsValue',]

const getCurrencySymbol = () => {
  return document.getElementById('fb-headbar-block-currency').children[1].textContent
    .split(' ')[0]
}

const countRoomsReturned = () => {
  return getAccomodationResults().length
}

const countRoomsSerched = () => {
  return parseInt(document.getElementById('fb-qs-summary-rooms-quantity').children[0].textContent
    .replace(/[\D]/g, ''))
}
...
const minimunPricePerNight = (bookedNights) => {
  ...
  getAccomodationResults().forEach(offersForTheRoom => {
    ...
    summaryCards[index] = [...offersForTheRoom.element.lastElementChild.children]
    const offersPerCard = [...summaryCards[index].map(child => {
...
const getAccomodationResults = () => {
  const results = [...document.getElementById('results-items').children]
...
const getOptionalInfo = (summaryCard) =>{
  ...
  let options = []
  elementOptions = [...document.getElementById(summaryCardsId).lastElementChild.children[offerIndex].children[0].children[0].children[2].children[0].children]
...
const start = () => {
  ...
  if (url.includes('?')) {
    ...
    const relevantData = {
      ...
      'minimun-price-night': `${minimunPricePerNight(params['nbNightsValue']).price} ${getCurrencySymbol()}`,
      'currency': params['currency'],
      'rooms-returned': countRoomsReturned(),
    ...
    if(getOptionalInfo(minimunPricePerNight(params['nbNightsValue'])).length > 0){
      relevantData['optional-services'] = getOptionalInfo(minimunPricePerNight(params['nbNightsValue']))
    }
